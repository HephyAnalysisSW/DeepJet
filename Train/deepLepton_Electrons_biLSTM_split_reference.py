...
if newtraining:
    from models import model_deepLeptonReference_biLSTM_split
    
    train.setModel(model_deepLeptonReference_biLSTM_split,dropoutRate=0.5,momentum=0.2)
    ...
    model,history = train.trainModel(nepochs=100, #3, #4    #small numbers for test runs, otherwise 100
                                     batchsize=2048, #64, #512, #1024, #2048, #4096
                                     stop_patience=300, 
                                     lr_factor=0.5, 
                                     lr_patience=5, 
                                     lr_epsilon=0.00001, 
