...
if newtraining:
    from models import model_deepLeptonReference_reduced
    #from models import model_deepLeptonReference
    
    train.setModel(model_deepLeptonReference_reduced,dropoutRate=0.5,momentum=0.1)
    #train.setModel(model_deepLeptonReference,dropoutRate=0.75,momentum=0.2)
    
    #train.keras_model=fixLayersContaining(train.keras_model, 'regression', invert=False)
    
    train.compileModel(learningrate=0.0001, #0.001,
    ...
    model,history = train.trainModel(nepochs=20, #3, #4 
                                     batchsize=16384, #64, #512, #1024, #2048, #4096
                                     stop_patience=300, 
                                     lr_factor=0.5, 
                                     lr_patience=2, 
                                     lr_epsilon=0.00005, 
