...
from buildingBlocks_deepLepton import block_deepLeptonConvolutions, block_deepLeptonDense, block_deepLeptonConvolutions_reduced, block_deepLeptonDense_reduced #, block_SchwartzImage, block_deepFlavourBTVConvolutions
...
def model_deepLeptonReference_reduced(Inputs,nclasses,nregclasses,dropoutRate=0.5,momentum=0.2):
    """
    reference 1x1 convolutional model for 'deepLepton'
    with recurrent layers and batch normalisation
    standard dropout rate it 0.1
    ...
    #
    npf = LSTM(20,go_backwards=True,implementation=2, name='npf_lstm')(npf)
    npf = BatchNormalization(momentum=momentum,name='npflstm_batchnorm')(npf)
    npf = Dropout(dropoutRate)(npf)
    
    cpf  = LSTM(60,go_backwards=True,implementation=2, name='cpf_lstm')(cpf)
    cpf = BatchNormalization(momentum=momentum,name='cpflstm_batchnorm')(cpf)
    cpf = Dropout(dropoutRate)(cpf)
    
    ppf = LSTM(30,go_backwards=True,implementation=2, name='ppf_lstm')(ppf)
    ppf = BatchNormalization(momentum=momentum,name='ppflstm_batchnorm')(ppf)
    ppf = Dropout(dropoutRate)(ppf)
    
    epf = LSTM(20,go_backwards=True,implementation=2, name='epf_lstm')(epf)
    epf = BatchNormalization(momentum=momentum,name='epflstm_batchnorm')(epf)
    epf = Dropout(dropoutRate)(epf)
    
    mpf = LSTM(20,go_backwards=True,implementation=2, name='mpf_lstm')(mpf)
    mpf = BatchNormalization(momentum=momentum,name='mpflstm_batchnorm')(mpf)
    mpf = Dropout(dropoutRate)(mpf)
    
    vtx = LSTM(10,go_backwards=True,implementation=2, name='vtx_lstm')(vtx)
    vtx = BatchNormalization(momentum=momentum,name='vtxlstm_batchnorm')(vtx)
    vtx = Dropout(dropoutRate)(vtx)
