...
#simple classes only prompt vs not prompt
class TrainDataDeepLepton_simple(TrainData):
    '''
    Base class for DeepJet.
    TO create own b-tagging trainings, please inherit from this class
    '''
...
class TrainData_fullTruth_simple(TrainDataDeepLepton_simple):
    def __init__(self):
        TrainDataDeepLepton_simple.__init__(self)
        self.clear()
        
    def reduceTruth(self, tuple_in):
        
        self.reducedtruthclasses=['lep_isPromptId_Training','lep_isNotPromptId_Training']
        if tuple_in is not None:
            prompt = tuple_in['lep_isPromptId_Training'].view(numpy.ndarray)
            notprompt = tuple_in['lep_isNotPromptId_Training'].view(numpy.ndarray)
